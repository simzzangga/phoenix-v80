...
# 하야부사 300km/h 배경화면 및 커스텀 스타일 적용
st.markdown(f"""
    <style>
    .stApp {{
    ...
    .stApp [data-testid="stHeader"] {{ background: rgba(0,0,0,0); }}
    section[data-testid="stSidebar"] {{ background: rgba(0,0,0,0.8) !important; }}
    .stMarkdown, .stText, p, h1, h2, h3, span {{ color: white !important; font-family: 'Courier New', Courier, monospace; }}
    div[data-testid="stExpander"] {{ background: rgba(0,0,0,0.5); border: 1px solid #D32F2F; }}
    .stButton>button {{ background-color: rgba(211, 47, 47, 0.8) !important; color: white !important; border-radius: 5px; border: none; }}
...
with col_h1:
    st.markdown("### 🟢 PHOENIX v5.9.88 [Aggressive Overload]")
with col_h2:
    if st.button("🔄", help="네트워크 및 캐시 리셋"):
        st.cache_data.clear()
...
with st.form("ignite_form"):
    search_input = st.selectbox("🎯 타겟 락온", krx_df['Display'].tolist())
    # 하야부사 스타트 버튼 컬러 (무광 레드)
...
if btn_click or st.session_state.auto_code:
    ...
    if res:
        st.session_state.last_viewed = t_code
        ...
        st.markdown(f"#### {res['상태']} : {name} ({t_code})")
        st.write(f"**적합도:** {res['적합도']} | **매수비중:** {res['금일비중']} | **익절:** {res['익절']} | **손절:** {res['손절']}")
        ...
        fig.update_layout(height=280, margin=dict(l=0,r=0,t=0,b=0), template="plotly_dark", 
...
if st.session_state.scan_storage:
    ...
    # 지휘관 요청 컬럼 순서 고정
    cols = ['날짜', '종목명', '종목코드', '적합도', '상태', '금일비중', '익절', '손절']
